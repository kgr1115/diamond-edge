...
## Custom Domain â€” diamond-edge.co

`diamond-edge.co` is the primary domain (purchased 2026-04-23 via Cloudflare registrar).

**Status: PURCHASED. DNS / Vercel wiring pending â€” see `docs/runbooks/domain-migration-diamond-edge-co.md` for the operational checklist.**

Summary of remaining wiring (full steps in runbook):

1. Vercel dashboard: Project â†’ Settings â†’ Domains â†’ Add `diamond-edge.co` and `www.diamond-edge.co`.
2. Cloudflare DNS: A record `@ â†’ 76.76.21.21`, CNAME `www â†’ cname.vercel-dns.com` (both DNS-only / gray cloud).
3. Add to `supabase/config.toml` â†’ `additional_redirect_urls` (already in the file comments; Supabase dashboard must mirror).
...
The Vercel GitHub integration handles deploy-on-push. There is no deploy step in CI â€” Vercel does it automatically.

**One-time setup (done by Kyle in Vercel dashboard):**

1. Go to [vercel.com/new](https://vercel.com/new)
2. Import the `diamond-edge` GitHub repository.
...
6. Vercel will also create Preview Deployments for every PR â€” useful for QA.
...
|---|---|---|---|
| `/api/cron/odds-refresh` | `0,30 12-23 * * *` | 8amâ€“7:30pm EDT every 30 min | Odds pull, daytime window |
| `/api/cron/odds-refresh` | `0,30 0-3 * * *` | 8pmâ€“11:30pm EDT every 30 min | Odds pull, evening window |
...
**Note:** Schedules are tuned for EDT (UTC-4), active during baseball season (Aprilâ€“October). In EST (Novemberâ€“March), these shift 1 hour. Since there is no MLB activity in the off-season, this is acceptable.

**Note:** The `odds-refresh` route handler must short-circuit immediately (return 200 with no API call) outside active game hours. The two cron entries provide full coverage of the 8amâ€“11pm ET window with no off-hours API calls.
...
- **Production** â€” live site, real API keys
- **Preview** â€” used for PR preview deployments
- **Development** â€” used when running `vercel dev` locally
