...
- **`picks` table writes:** The pipeline must determine `required_tier` for each pick:
  ...
  - Note: all published picks (tier 3+) have `required_tier` of at least 'pro'. There are no free-tier published picks from the pipeline. Free and anon users see the same live picks on the slate with tier-gated fields (price, rationale, EV, SHAP, line snapshots) masked by `maskPick` and the `PickCard` paywall teaser ("Unlock full analysis with Pro") rendered in place of the locked content — consistent with TASK-008. Do NOT filter anon/free viewers to `required_tier='free'` in the loader; the pipeline never emits such picks and that filter produces an empty slate.
