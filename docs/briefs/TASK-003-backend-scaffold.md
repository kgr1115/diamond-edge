...
- Repo root: `C:\AI\Public\diamond-edge`. ADR-001 defines the folder layout.
- Stack: Next.js 15 App Router + TypeScript + Tailwind CSS + shadcn/ui (locked in CLAUDE.md).
- DB: Supabase Postgres with RLS. Every user-facing table has RLS â€” no exceptions (schema design principle).
...
All inputs are checked in at the repo root `C:\AI\Public\diamond-edge`:

- `CLAUDE.md` â€” locked stack and engineering principles
- `docs/schema/schema-v1.md` â€” 13-table Postgres schema with full RLS policies and enums
- `docs/api/api-contracts-v1.md` â€” 13 typed API routes (response shapes, auth requirements, error envelope)
- `docs/schema/caching-strategy.md` â€” Redis key patterns, TTLs, invalidation triggers per route
- `docs/compliance/geo-block-spec.md` â€” Edge Middleware logic, conflict resolution table, ALLOW list
- `docs/compliance/age-gate-spec.md` â€” DOB gate mechanics, failure behavior, audit log spec
- `docs/adr/ADR-001-repo-structure.md` â€” canonical folder layout

---

## Deliverable Format

Working code committed to `C:\AI\Public\diamond-edge`:

1. **`apps/web/`** â€” scaffolded Next.js 15 app. Must include:
   ...
   - `app/page.tsx` (placeholder homepage â€” "Diamond Edge coming soon")
   - `middleware.ts` â€” Vercel Edge Middleware implementing geo-block logic per geo-block-spec.md
   - `lib/supabase/` â€” server client, browser client, middleware client (three helper files per @supabase/ssr pattern)
   - `lib/redis/cache.ts` â€” thin typed wrapper: `get<T>()`, `set<T>()`, `invalidate()` per caching-strategy.md

2. **`supabase/migrations/`** â€” numbered SQL migration files implementing every table, enum, index, and RLS policy from schema-v1.md:
   - `0001_enums.sql` â€” all 5 enums
   - `0002_lookup_tables.sql` â€” sportsbooks (with seed data: DK + FD), geo_blocked_states (seeded with all non-ALLOW states)
   - `0003_reference_tables.sql` â€” teams, players
   - `0004_core_tables.sql` â€” games, odds, rationale_cache
   - `0005_pick_tables.sql` â€” picks, pick_outcomes
   - `0006_user_tables.sql` â€” profiles (with auth trigger), subscriptions, bankroll_entries, age_gate_logs
   - `0007_rls_policies.sql` â€” all RLS policies (can be inline with tables above OR a separate file â€” your call, but name it clearly)

3. **`apps/web/app/api/auth/age-verify/route.ts`** â€” POST handler per age-gate-spec.md:
   ...
   - Same 403 for both "format invalid" and "age < 21" â€” no leakage

4. **`apps/web/app/api/webhooks/stripe/route.ts`** â€” Stripe webhook stub:
...
5. **`apps/web/app/api/picks/today/route.ts`** â€” GET handler:
...
6. **`supabase/config.toml`** â€” project config with dev/staging/prod awareness
...
- `docs/schema/schema-v1.md` â€” DONE (TASK-001)
- `docs/api/api-contracts-v1.md` â€” DONE (TASK-001)
- `docs/schema/caching-strategy.md` â€” DONE (TASK-001)
- `docs/compliance/geo-block-spec.md` â€” DONE (TASK-002)
- `docs/compliance/age-gate-spec.md` â€” DONE (TASK-002)

**Does NOT require:**
- DevOps provisioning (TASK-006) â€” write code against local Supabase dev; DevOps wires prod secrets later
- Data ingestion (TASK-004) â€” build against the schema shape; actual data comes later
- ML model (TASK-005) â€” routes that call the pick pipeline are stubs (return mock data) until TASK-005 completes

**This task unblocks:**
- TASK-007 (AI Reasoning) â€” needs stable API routes
- TASK-008 (Frontend) â€” needs stable API routes and Supabase client helpers
- TASK-009 (Stripe billing) â€” needs auth + subscriptions table
