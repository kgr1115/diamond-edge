...
- Urgency badge next to game time: countdown pill ("in 2h 14m") for scheduled games (neutral > 2h, amber < 2h, red < 30m) and state pill ("Live" / "Final" / "PPD" / "Cancelled") for non-scheduled games. Pure logic lives in `lib/picks/urgency.ts` (`resolveUrgency`) — single source of truth, unit-tested in `lib/picks/__tests__/urgency.test.ts`. The badge is informational only; no CTA copy anywhere on the card.
- **Line-locked treatment** (live / final / postponed / cancelled — `UrgencyState.lineLocked === true`): in addition to the header pill and `opacity-60`, the card renders an unmissable top-of-card banner replacing the standard border with the factual reason — `"GAME IN PROGRESS — line locked"` (live, amber), `"GAME FINAL — line closed"` (final, gray), `"GAME POSTPONED — line voided"` / `"GAME CANCELLED — line voided"` (gray). The displayed price is rendered with `line-through` strikethrough in muted gray. Card border switches to dashed gray and `cursor-not-allowed` is applied to signal the line is not actionable. The wrapping `<Link>` stays active (informational click-through to pick detail). Copy is intentionally factual ("line locked / closed / voided") rather than directive — it describes the market state, not a betting recommendation.
