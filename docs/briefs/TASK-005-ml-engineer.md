# TASK-005 — ML Feature Engineering + Model Design
...
Define the full feature engineering scope and select the statistical model(s) for moneyline, run-line, and totals markets (no parlays in v1); make and justify the Fly.io worker vs. Supabase Edge Function inference runtime decision with a cost estimate; and design the backtesting harness — all producing output that exactly matches the PickCandidate schema in `docs/api/ml-output-contract.md`.

---

## Context

- Markets in scope for v1: moneyline, run_line, total. Props are v1 stretch (include if clean). Parlays explicitly deferred to v1.1 — do not scope parlay EV.
- Minimum confidence threshold for publication: `confidence_tier >= 3` (EV > 4%). LOCKED DECISION. Your model must produce calibrated confidence tiers 1–5 per the tier derivation table in `ml-output-contract.md`.
- Free tier users see NO rationale and NO model data. Pro tier: model_probability. Elite tier: model_probability + expected_value + SHAP. Your output schema feeds all of these — tier gating happens downstream at the API layer.
...
- Budget: $300/mo total. Statistical model must be CPU-servable (gradient boosting, logistic regression, XGBoost, LightGBM). GPU is not approved for v1 — flag explicitly if you believe it's required.
- Vercel function timeout is 10s (Hobby) / 60s (Pro). If inference + feature lookup exceeds this, it goes to Fly.io worker. Supabase Edge Functions (Deno) don't support Python natively — Python models must run on Fly.io. This is the key runtime fork.
...
- `docs/api/ml-output-contract.md` — the PickCandidate schema your model must produce exactly; the confidence tier derivation table; the pick pipeline seam diagram
- `docs/schema/schema-v1.md` — `games`, `odds`, `teams`, `players` tables (your input data lives here)
- `docs/api/api-contracts-v1.md` — downstream consumers (tier gating table shows which model outputs go to which tier)

---

## Deliverable Format

Artifacts committed under the repo root `C:\Projects\Baseball_Edge`:

Per ADR-001, model artifacts live under `worker/` (the Fly.io Python worker directory).

1. **`worker/models/README.md`** — Top-level model overview:
   ...
   - Runtime decision: Fly.io worker vs. Supabase Edge Function — written up with cost rationale

2. **`worker/models/moneyline/feature-spec.md`** — Feature engineering document:
...
3. **`worker/models/run_line/feature-spec.md`** — Same format (can share many features with moneyline)

4. **`worker/models/totals/feature-spec.md`** — Same format (weather and park factor are especially important here)

5. **`worker/models/backtest-harness.md`** — Backtesting spec:
   - Date range: minimum 3 MLB seasons (2021–2023 historical; 2024 holdout)
...
6. **`worker/models/calibration-spec.md`** — Confidence tier mapping:
   - Final mapping from EV + uncertainty → confidence_tier 1–5
...
7. **`worker/models/inference-runtime.md`** — Runtime decision document:
...
8. **`worker/models/pick_candidate_schema.py`** — Python dataclass or TypedDict matching `ml-output-contract.md` exactly:
...
- [ ] `inference-runtime.md` commits to Fly.io OR Edge Function with cost estimate — does not leave it open.
...
- `docs/api/ml-output-contract.md` — DONE (TASK-001): PickCandidate schema defined
- `docs/schema/schema-v1.md` — DONE (TASK-001): input data tables defined

**Does NOT require:**
- TASK-004 complete — you design against the schema; actual data comes from TASK-004 pipeline
- TASK-006 complete — you specify the runtime; DevOps provisions it

**This task unblocks:**
- TASK-006 (DevOps) — needs runtime decision to provision Fly.io or confirm Edge Function suffices
- TASK-007 (AI Reasoning) — needs feature_attributions spec to write grounded rationale prompts
- Pick pipeline implementation (Phase 2) — needs the model to actually run

**New secrets/env vars to surface to DevOps (if Fly.io chosen):**
- `FLY_APP_NAME` — for the ML worker
- `MODEL_ENDPOINT_URL` — URL of the Fly.io prediction endpoint
