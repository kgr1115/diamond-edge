...
Provision Vercel + Supabase + Upstash projects across dev/staging/prod environments, write the GitHub Actions CI skeleton, set up secrets management, and wire a cost dashboard — all within the $300/mo budget envelope ($79/mo already committed to The Odds API, leaving ~$221/mo for everything else).
...
- Fly.io: provision a minimal `fly.toml` skeleton for the ML/LLM inference worker — even if TASK-005 hasn't confirmed the runtime yet, scaffold it so it's ready. Use the smallest viable instance (1 shared CPU, 256MB RAM as starting point). If TASK-005 confirms Edge Function is sufficient, Fly.io skeleton stays but isn't deployed.
- Vercel plan: start with Hobby tier to confirm whether 10s function timeout is sufficient. Flag to orchestrator if any route from api-contracts-v1.md exceeds 10s (particularly the pick pipeline trigger and LLM rationale generation). If Pro is needed, cost delta is ~$20/mo — document and flag.
...
- `CLAUDE.md` — locked stack, budget envelope
- `docs/adr/ADR-001-repo-structure.md` — folder layout to wire CI against
- `docs/schema/caching-strategy.md` — Upstash usage patterns (informs Redis tier selection)
- `docs/api/api-contracts-v1.md` — cron routes to register in `vercel.json`

---

## Deliverable Format

Config files and documentation committed to `C:\Projects\Baseball_Edge`:
...
2. **`.github/workflows/ci.yml`** — GitHub Actions CI:
   ...
   - On merge to main: trigger Vercel deploy (via Vercel GitHub integration, not a manual curl — just document that the integration must be set up)

3. **`.github/workflows/migrations.yml`** — Supabase migration safety:
   ...
   - Prod migrations: manual trigger only (safety gate) — document the command in the workflow as a commented-out step

4. **`worker/fly.toml`** — Fly.io ML worker skeleton (per ADR-001, worker config lives in `worker/`):
   - App name: `diamond-edge-worker` (or equivalent)
   - 1 shared CPU, 256MB RAM (smallest viable; ML agent may upsize)
   - Health check endpoint: `GET /health`
   - Scale to zero when not in use (important for cost control — document how)

5. **`docs/runbooks/`** — Three runbooks:
   - `odds-ingestion-lag.md` — what to do if odds haven't refreshed in >1 hour
   - `pick-pipeline-failure.md` — what to do if no picks appear by 10am ET
   - `cost-spike.md` — what to do if Vercel/Supabase/Upstash spend spikes unexpectedly

6. **`docs/infra/cost-projection.md`** — Cost breakdown:
...
7. **`docs/infra/secrets-manifest.md`** — Complete list of all env vars:
...
8. **`supabase/config.toml`** (if not already written by TASK-003 — coordinate):
...
- [ ] `worker/fly.toml` skeleton committed (deploy not required — just the config file).
...
- [ ] Upstash $20/mo budget alert configured (document the step to do this in the Upstash console — you may not be able to automate it via config).
...
- `docs/adr/ADR-001-repo-structure.md` — DONE (TASK-001): folder structure known
- `docs/api/api-contracts-v1.md` — DONE (TASK-001): cron routes defined

**Partial dependency (can start without, update when available):**
- TASK-005 runtime decision (Fly.io vs. Edge) — scaffold both; update fly.toml when TASK-005 resolves
...
- `CRON_SECRET` — shared secret for all Vercel Cron route handlers
- `NEXT_PUBLIC_SUPABASE_URL` — Supabase project URL (public, safe in client bundle)
- `NEXT_PUBLIC_SUPABASE_ANON_KEY` — Supabase anon key (public, safe in client bundle)
- `SUPABASE_SERVICE_ROLE_KEY` — server-only, never in client bundle
- `GEO_ALLOW_STATES` — comma-separated state codes: AZ,AR,CO,CT,DC,IL,IN,IA,KS,KY,LA,MD,MA,MI,MO,NJ,NY,NC,OH,PA,TN,VT,VA,WV,WY
